...
class Role(db.Model):
    ...
    def insert_roles():
        roles = {
            'Public': (Permission.PUBLIC, 'main', True),
            'Student': (Permission.STUDENT, 'main', False),
            'Faculty': (Permission.FACULTY, 'main', False),
            'Student Organization Leader': (Permission.ORG_LEADER, 'main', False),
            'Reservation Organizer': (Permission.ORGANIZER, 'admin', False),
...
class User(UserMixin, db.Model):
    ...
    def __init__(self, **kwargs):
        ...
        if self.role is None:
            ...
            if "@tufts.edu" in self.email:
                self.role = Role.query.filter_by(permissions=Permission.STUDENT).first()
            if self.role is None:
    ...
    def has_dashboard(self):
        return self.can(Permission.ORGANIZER)

    def is_role_manager(self):
        return self.can(Permission.ROLE_MANAGER)
...
class Role_Request(db.Model):
    ...
    status = db.Column(db.Integer, db.ForeignKey('request_statuses.id'))
